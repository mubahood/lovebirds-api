...
use Illuminate\Database\Eloquent\Factor    public function chatHead()
    {
...
class UserMatch extends Model
{
    ...
    public function chatHead()
    {
        ...
        return $this->hasOne(ChatHead::class, function($query) use ($userId, $matchedUserId) {
            $query->where(function($q) use ($userId, $matchedUserId) {
                $q->where('customer_id', $userId)
                  ->where('product_owner_id', $matchedUserId);
            })->orWhere(function($q) use ($userId, $matchedUserId) {
                $q->where('customer_id', $matchedUserId)
                  ->where('product_owner_id', $userId);
            })->where('type', 'dating');
        });
    }
